...
module Enkaironment
  class Install < Thor
    ...
    def zsh(username)
      install 'zsh'
      system %(chsh -s `which zsh` #{username})
    end
    ...
    def prezto(username)
      ...
      Git.clone('https://github.com/sorin-ionescu/prezto.git', '.zprezto', path: user.dir)
      symlink_prezto(user.dir, preztodir)
      FileUtils.chown_R(user.uid, user.gid, Dir["#{user.dir}/.*", "#{user.dir}/*"])
    end
    ...
    def rbenv(username)
      ...
      user = Etc.getpwnam(username)
      system %(su #{username} -c "git clone https://github.com/rbenv/rbenv.git #{user.dir}/.rbenv")
      rbenv_shim = <<~RBENV
    ...
    def rbenv_build(username)
      ...
      %w[git rbenv ruby_build_deps].map(&method(:invoke))
      system %(su #{username} -c "git clone \
    ...
    def ruby(username, version = '2.5.1')
      system %(su #{username} -c "rbenv install #{version}")
    end
    ...
    def spacevim(username)
      system %(su #{username} -c "#{`curl -sLf https://spacevim.org/install.sh`}")
    end
